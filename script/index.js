...
function preload() {
    ...
    this.load.spritesheet('dude',
        'files/dude.png',
        {frameWidth: 31.5, frameHeight: 34}
    );
...
function create() {
    ...
    for (let i = 1; i <= 11; i += 2) {
        for (let j = 1; j <= 11; j += 2) {
            blocks.create((document_width / 11) * i + demi, (document_height / 11) * j + demi, 'caisse').setDisplaySize(gameWidth / 12, gameWidth / 12);
        }
    }

    player = this.physics.add.sprite(demi, demi, 'dude');
    player.setCollideWorldBounds(true);
    player.setDisplaySize( gameWidth / 12, gameWidth / 12);

    player2 = this.physics.add.sprite(document_width - demi, document_height - demi, 'dude2');
    player2.setCollideWorldBounds(true);
    player2.setDisplaySize( gameWidth / 12, gameWidth / 12);
    player2.setX(1);
